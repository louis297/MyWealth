...
| [transactions](transactions.md) | accepted | Phase 3 | Top-level `/transactions`. Append-only create + list/get. Buy/Sell auto-adjust Holding (average cost) inside Account aggregate. Cash types leave Holdings untouched. Amount always > 0. Holding-delete guard (no delete if historical Tx) delivered here. API only. |
| [dashboard](dashboard.md) | accepted | Phase 4 | Net Worth + asset allocation (pure read models). Multi-currency arrays, optional customerId on both endpoints, Closed excluded, Credit = liability, signed-sum locked, API only. |
