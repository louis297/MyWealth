...
| [holdings](holdings.md) | accepted | Phase 3 | Nested under `/accounts/{accountId}/holdings`. Create/list/get/update/delete. Quantity ≥ 0 (incl. zero). CostBasis.Currency immutable & must match Account. No list search/pagination. Physical DELETE allowed; “no delete if historical Tx” guard is delivered by the Transactions feature. API only. |
